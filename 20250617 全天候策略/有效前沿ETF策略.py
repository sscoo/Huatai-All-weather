...
class EfficientFrontierETF:
    ...
    def save_monthly_results(self, start_date, end_date, lookback_months):
                ...
                if strategy in self.monthly_backtest_results:
                    ...
                    for date in cum_returns.index:
                        daily_returns_data.append({
                            ...
                            '策略名称': name,
                            '策略代码': strategy,
                            '累计收益率': f"{cum_returns.loc[date]:.6f}",
                            '累计收益率百分比': f"{(cum_returns.loc[date]-1)*100:.2f}%",
                            '当日收益率': f"{daily_returns.loc[date]:.6f}" if date in daily_returns.index else "0.000000",
                            '当日收益率百分比': f"{daily_returns.loc[date]*100:.4f}%" if date in daily_returns.index else "0.0000%"
                        })
        ...
        print("   3. 月度调仓每日收益明细.csv - 每日收益率变化")
